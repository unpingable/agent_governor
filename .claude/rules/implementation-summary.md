...
**Runtime Supervisor** — Governed sessions over external agent CLIs. Canonical event bus (JSONL persistence, monotonic seq, tool_call_id dedup, cursor reads). RuntimeAdapter protocol with declared capabilities. Claude Code adapter (supervised mode, Unix socket hooks, PascalCase hook format). SessionSupervisor with lifecycle FSM (created→launching→attaching→running→exited/failed), intervention queue with configurable timeouts, auto-approve for read-only tools. Promotions: workspace diff detection via git, approve/reject/revert, junk file filtering. Session forking from promoted parents with explicit lineage. Settings cleanup (atexit handler + CLI command). 13 daemon RPC methods (runtime.session.*, runtime.intervention.*, runtime.promotion.*), 79 total. CLI: `governor runtime {launch,fork,list,events,interventions,approve,deny,promotion,diff,promote,reject,kill,cleanup}`. Dogfood-verified: 6 supervised Claude Code sessions including denial, timeout, and full promotion workflow. Maude integration: 12 `supervised` commands + `snapshot`/`wtf` operator overview.
