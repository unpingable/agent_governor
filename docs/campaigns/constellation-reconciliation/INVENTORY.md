...
| cli.md: "Apply verified proposal" (1) · resolve/rule/except wording as if CLI = ruling (3, ×3) · override creation without named authority (1) · anchor `--type canon` (6) · `autonomous run` blurs consumer/actor (4 — sweep's WORST) · `tune --apply` (1) · glossary PROPOSED-name-in-copy (1) | 1,3,4,6 | **UPGRADED per A9 → EXECUTED 8/9 (2026-07-04)**: `cli-reference-wording-pass` run — labels reworded in cli.md + the cli-reference.md mirror (decisions attributed to the operator; "apply" describes mechanics; `--type canon` got a clarifying note, flag unchanged). **9th finding NOT executed:** the "glossary PROPOSED-name-in-copy" quote did not survive this table's condensation (A2 acceptance said zero findings without quotes — a completeness debt of the A2 *record*, distinct from the sweep); refusing to guess the target; recoverable only by re-running the A2 sweep over cli.md. Backlog stub `executed_partial`. **9th finding CLOSED no-current-target (2026-07-15):** the prescribed recovery was executed — the sweep re-run mechanically over the glossary's complete PROPOSED ops-term set (16 terms) against `docs/interfaces/cli.md`, the `cli-reference.md` mirror, and the glossary's own prose: ZERO instances of a PROPOSED name used as copy. The lone textual collision (`governor gate check`) is the native CLI command name, not the PROPOSED rename of wicket admission. The 8/9 commit `6238c99` removed no PROPOSED term, so the target was not incidentally fixed — it has no surviving referent. Closed on the negative re-sweep result, not a guess; if a future A2-style sweep surfaces a quoted instance, it enters as a NEW finding with its quote intact. Backlog stub `done`. `autonomous run` + `anchor --type canon` remain flagged to the governed-shell campaign. |
