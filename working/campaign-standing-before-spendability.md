...
9. **the source of its own work** (added 2026-06-10 per endgame-synthesis
   trap analysis). The LLM may propose edits within an operator-curated
   intent, but it must not mint new backlog items, expand its own mandate,
   or create follow-on work for other unattended agents. Agent-generated
   backlog consumed by other agents is slop recursion wearing a tiny hard
   hat — the compost heap that invoices you. Allowed work sources:
   operator-written notes, NQ findings, explicit watchbills,
   human-curated backlog items.
...
| **D0f-docs (initial pass)** | **CLOSED 2026-06-10** | Sprint closure documentation, initial pass. Seven docs landed under `docs/{demo,architecture,reference,notes}`: standing-before-spendability.md, wal-bloat-drill-transcript.md, claim-custody-spine.md, origin-mode.md, refusal-and-outcome-vocabulary.md, drill-scenarios.md, sprint-receipt.md. All 10 operator acceptance criteria met using verbatim ratified vocabulary copied from drill_runner / drill_poster / linear_accountant_client / cooked_context_orchestrator. No source/test changes; 202 slice tests still pass. Tier-2 cadence held (one careful review pass per doc; no doctrine re-litigation). |
| **D0f-prep**     | **CLOSED 2026-06-10** | Glossary / endgame doctrine alignment between initial D0f-docs and final closure. Added the ninth LLM must-not ("source of its own work") to §3b. Sharpened the "no self-amendment" framing to **directional custody** (downstream capability cannot mutate upstream authorization while inside the downstream flow). Sharpened BA3 thaw trigger address: Maude write-tool approval IS the convertible spend boundary. Created `docs/reference/internal-ops-glossary.md` (living glossary, 8 operator-proposed + 6 carried + 7 universal + 2 internal-only). Unparked `working/parked-constellation-alignment-pass.md` with the bilingual refinement (glossary maintenance, not rip-and-replace). No code/internal-symbol renames; no PROPOSED rows ratified; no Maude/LA wiring dispatched. |
| **D0f-docs (sharpening pass)** | **CLOSED 2026-06-10** | Surgical refresh complete. `docs/architecture/claim-custody-spine.md` got: (a) the ninth LLM must-not ("the source of its own work") added verbatim to the §3b copy of the must-not list; (b) a "Directional custody (why these must-nots exist)" paragraph explaining the underlying invariant; (c) a vocabulary note pointing to the glossary with explicit "PROPOSED rows are not yet binding" framing. The other six docs reviewed; no stale framings found (doctrine sentence "the model may propose..." stays intact in origin-mode.md and standing-before-spendability.md; sprint-receipt.md left as point-in-time inventory). No code/internal-symbol renames; no PROPOSED rows ratified; no scope expansion. |
| Sprint commit    | **PUSHED 2026-06-10** | One commit per repo (NQ `428828a` / scheduler `d38d890` / AG `07768e8`), all narrative-unit messages, all pushed clean to origin/main on github-unpingable. AG pre-commit hook passed; six pre-existing operator working notes left unstaged. |
| **Endgame synthesis** | **FILED 2026-06-10 (doctrine deposit, NOT a build directive)** | Recognition record at `working/endgame-synthesis-2026-06-10.md`. After the sprint commit, Fable + operator synthesized the constellation's endgame: Maude is already the supervised runtime; the missing layer is policy-mode answering the approval prompt; LA wires at Maude's tool-call dispatcher (the "convertible spend path appears" trigger); Phosphor renders the morning review queue. Genesis-class constitutional rule proposed: *system may not self-amend the gates that authorize/meter/witness/classify/enforce its own actions.* No agents dispatched off this file; operator pacing governs next move. |
