...
| Tock 2 | **shipped, tested** | `working/tock-02-session-attributable-promotion.md` — session-attributable promotion; forcing gap GAP-N. Baseline snapshot at launch, refuse-dirty-by-default (`allow_dirty` to fence), per-path `revert_paths` (reject can't destroy pre-existing work). 16 tests incl. back-to-back-dirty regression; 499-test runtime sweep green. |
