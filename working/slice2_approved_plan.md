# APPROVED PLAN (outer review, 2026-06-17) — slice 2 capacity_refused legibility

Outer controller APPROVES the design pass below. Two load-bearing constraints —
honor them exactly:

  (1) Assign the disposition at the ONE branch where InsufficientCapacity is
      unambiguous (inside consume()), and CARRY it through RefusalResult ->
      LabEffectDecision -> payload. NEVER derive retry_disposition downstream from
      la_kind == "capacity_refused" (four sources collapse to that token; only the
      consume/InsufficientCapacity source earns new_authority_required).
  (2) The no_session_grant path (lab_gate) can itself wrap a request-time
      capacity_refused — it must default to retry_disposition=unknown, NOT inherit
      new_authority_required.

Implement the plan as written. Make COHESIVE edits (8-unit grant, no top-up).
Add the tests it lists. Do NOT run Bash/tests (outer runs them). Then STOP.

----- design plan (verbatim from the read-only pass) -----
